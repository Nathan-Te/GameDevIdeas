...
const { makeApp, guest, post, seedIdea, get } = await import('./helpers.js');
...
test('un invité voit les fichiers de la sélection, pas les autres', async (t) => {
  ...
  assert.equal((await guest(app, 'GET', `/files/${visible}`)).status, 200);

  const refuse = await guest(app, 'GET', `/files/${cachee}`);
...
test('révoquer la sélection referme ses fichiers', async (t) => {
  ...
  assert.equal((await guest(app, 'GET', `/files/${relative}`)).status, 200);

  await post(app, `/api/shares/${share.id}/revoke`, {});

  assert.equal((await guest(app, 'GET', `/files/${relative}`)).status, 404);
...
test('un fichier sans ligne en base n’est servi à aucun invité', async (t) => {
  ...
  assert.equal((await guest(app, 'GET', `/files/${orphelin}`)).status, 404);
});
